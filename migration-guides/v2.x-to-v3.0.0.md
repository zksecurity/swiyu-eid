...
This guide helps you migrate from **v2.x** to **v3.0.0** of the Swiyu Verifier Service.

It is based on the **v3.0.0** section in `CHANGELOG.md` and the migration to the hardened
`dhi.io` base image (`#834`).

## Compatibility summary

- **Wallets / OID4VP clients:** no changes required beyond those already documented in the
  v3.0.0 `CHANGELOG.md` entry (DCQL-only, removed SWIYU-API-Version 1).
...
v3.0.0 ships **two image variants** so you can decouple the application upgrade from the
runtime hardening:

| Variant | Tag pattern                                              | When to pick it |
|---|----------------------------------------------------------|---|
| **Hardened (default)** | `ghcr.io/swiyu-admin-ch/swiyu-verifier:<tag>`            | New deployments; operators who have completed the steps below; anyone who wants the smaller attack surface and `nonroot` runtime now. |
| **Unhardened (transitional)** | `ghcr.io/swiyu-admin-ch/swiyu-verifier:<tag>-unhardened` | Existing deployments that still rely on the shell entrypoint (`HTTP_PROXY`/`HTTPS_PROXY`/`NO_PROXY`, `MY_SPRING_PROFILES`, `JAVA_BOOTCLASSPATH` + `/lib` JCE mounts). Buys you time to migrate, but **will be removed in a later release** — plan your move. |
...
```diff
- image: ghcr.io/swiyu-admin-ch/swiyu-verifier:3.0.0
+ image: ghcr.io/swiyu-admin-ch/swiyu-verifier:3.0.0-unhardened
```
...
### 1. Proxy variables: `HTTP_PROXY` / `HTTPS_PROXY` / `NO_PROXY` → `JAVA_TOOL_OPTIONS`
...
### 2. Spring profiles: `MY_SPRING_PROFILES` → `SPRING_PROFILES_ACTIVE`
...
### 3. Bootclasspath extras: `JAVA_BOOTCLASSPATH` / `/lib` volume → custom image layer
...
```dockerfile
FROM ghcr.io/swiyu-admin-ch/swiyu-verifier:3.0.0
USER 0
...
2. **No package manager** and a **minimal native runtime**. System libs many vendor PKCS#11
   ...
   re-hydrated by the project `Dockerfile` for the didresolver native library.
...
- Any **transitive `.so` dependencies** the vendor library needs. Check with
  ...
  builder stage the same way the project `Dockerfile` rehydrates `libgcc_s.so.1`.
...
# Stage 2: layer onto the hardened verifier image
FROM ghcr.io/swiyu-admin-ch/swiyu-verifier:3.0.0
USER 0
...
`HSM_PROXY_PASSWORD`, `HSM_USER_PIN`, `HSM_KEY_ID`, `HSM_KEY_PIN` environment variables and
...
- [ ] Update every image reference in your deployment manifests
  ...
  carries the `-unhardened` suffix (e.g. `ghcr.io/swiyu-admin-ch/swiyu-verifier:3.0.0-unhardened`).
...
- [ ] If using an HSM: build a derived image that bakes in the vendor JCE JAR and/or the
  ...
  provider.
- [ ] Smoke-test the deployment: the verifier should reach upstream registries through the
  configured proxy, and the active Spring profiles should appear in the startup banner.

## Reference

- v3.0.0 entry in [`CHANGELOG.md`](../CHANGELOG.md).
