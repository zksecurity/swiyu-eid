...
template CantonBase64UrlDecodedLength(maxEncoded, maxDecoded) {
...
template CantonAssertBase64UrlCharStrict() {
    signal input char;
...
template CantonBase64UrlSextet() {
    signal input char;
    signal output value;

    component strict = CantonAssertBase64UrlCharStrict();
    strict.char <== char;
    strict.enabled <== 1;
...
template CantonAssertBase64UrlCanonical32() {
    signal input encoded[43];

    component strict[43];
    for (var i = 0; i < 43; i++) {
        strict[i] = CantonAssertBase64UrlCharStrict();
        strict[i].char <== encoded[i];
        strict[i].enabled <== 1;
    }
    component last = CantonBase64UrlSextet();
...
template CantonAssertBase64UrlCanonicalTail(maxEncoded, maxDecoded) {
    signal input encoded[maxEncoded];
    signal input encodedLength;
    signal input decodedLength;

    component relation = CantonBase64UrlDecodedLength(maxEncoded, maxDecoded);
...
template CantonAssertSha256Padding(maxLen) {
...
///   [salt, "resident_canton", "ZH"]
/// and exposes only the canonical two-byte canton code plus the SHA-256 digest
/// used for membership in the credential's signed top-level `_sd` array.
template SwiyuCantonDisclosure() {
    var ENCODED_MAX = 128;
    var DECODED_MAX = 96;
    var HASH_TAIL = 120;
    var NAME_LEN = 15;
    ...
    var NAME[NAME_LEN] = [114,101,115,105,100,101,110,116,95,99,97,110,116,111,110];
    ...
    signal suffix18 <== SelectArrayValue(DECODED_MAX)(decoded, suffixStart + 18, 1); suffix18 === 34;
    signal suffix19 <== SelectArrayValue(DECODED_MAX)(decoded, suffixStart + 19, 1); suffix19 === 44;
    signal suffix20 <== SelectArrayValue(DECODED_MAX)(decoded, suffixStart + 20, 1); suffix20 === 34;

    signal cantonBytes[2];
    cantonBytes[0] <== SelectArrayValue(DECODED_MAX)(decoded, suffixStart + 21, 1);
    cantonBytes[1] <== SelectArrayValue(DECODED_MAX)(decoded, suffixStart + 22, 1);
    signal valueClose <== SelectArrayValue(DECODED_MAX)(decoded, suffixStart + 23, 1); valueClose === 34;
    signal arrayClose <== SelectArrayValue(DECODED_MAX)(decoded, suffixStart + 24, 1); arrayClose === 93;

    cantonCode <== cantonBytes[0] * 256 + cantonBytes[1];
    // AG AI AR BE BL BS FR GE GL GR JU LU NE NW OW SG SH SO SZ TG TI UR VD VS ZG ZH
    var CANTONS[26] = [16711,16713,16722,16965,16972,16979,18002,18245,18252,18258,19029,19541,20037,20055,20311,21319,21320,21327,21338,21575,21577,21842,22084,22099,23111,23112];
    component isCanton[26];
    signal cantonMatches[27];
    cantonMatches[0] <== 0;
    for (var i = 0; i < 26; i++) {
        isCanton[i] = IsEqual();
        isCanton[i].in[0] <== cantonCode;
        isCanton[i].in[1] <== CANTONS[i];
        cantonMatches[i + 1] <== cantonMatches[i] + isCanton[i].out;
    }
    cantonMatches[26] === 1;
