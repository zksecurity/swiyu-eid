...
include "swiyu/residence-disclosures.circom";
include "swiyu/p256.circom";
include "swiyu/split-commitments.circom";

/// Residence-eligibility Prepare relation.  It authenticates two exact
/// SD-JWT disclosures from one issuer-signed credential: the current main
/// residence's BFS municipality and its uninterrupted arrival date.
template SwiyuResidencePrepareBase(
    maxMessageLength,
    maxB64HeaderLength,
    maxB64PayloadLength
) {
    ...
    signal input municipalityDisclosurePadded[128];
    signal input municipalityDisclosureLength;
    signal input municipalityDisclosureJsonLength;
    signal input municipalityDisclosureSaltLength;
    signal input municipalityDigitLength;

    signal input sinceDisclosurePadded[128];
    signal input sinceDisclosureLength;
    signal input sinceDisclosureJsonLength;
    signal input sinceDisclosureSaltLength;
    ...
    signal input municipalityDisclosureDigestB64[43];
    signal input sinceDisclosureDigestB64[43];
    ...
    signal input payloadMunicipalityDigestStart;
    signal input payloadSinceDigestStart;
    ...
    component municipalityDigestMember = JsonDigestArrayMember(maxPayloadLength, 43);
    municipalityDigestMember.json <== payload;
    municipalityDigestMember.inStringBefore <== payloadStructure.inStringBefore;
    municipalityDigestMember.depthBefore <== payloadStructure.depthBefore;
    municipalityDigestMember.digest <== municipalityDisclosureDigestB64;
    municipalityDigestMember.digestStart <== payloadMunicipalityDigestStart;
    municipalityDigestMember.arrayOpen <== sdArray.openIndex;
    municipalityDigestMember.arrayClose <== payloadSdClose;
    component sinceDigestMember = JsonDigestArrayMember(maxPayloadLength, 43);
    sinceDigestMember.json <== payload;
    sinceDigestMember.inStringBefore <== payloadStructure.inStringBefore;
    sinceDigestMember.depthBefore <== payloadStructure.depthBefore;
    sinceDigestMember.digest <== sinceDisclosureDigestB64;
    sinceDigestMember.digestStart <== payloadSinceDigestStart;
    sinceDigestMember.arrayOpen <== sdArray.openIndex;
    sinceDigestMember.arrayClose <== payloadSdClose;

    component municipalityDisclosure = SwiyuResidenceMunicipalityDisclosure();
    municipalityDisclosure.encodedPadded <== municipalityDisclosurePadded;
    municipalityDisclosure.encodedLength <== municipalityDisclosureLength;
    municipalityDisclosure.decodedLength <== municipalityDisclosureJsonLength;
    municipalityDisclosure.saltLength <== municipalityDisclosureSaltLength;
    municipalityDisclosure.digitLength <== municipalityDigitLength;
    component sinceDisclosure = SwiyuResidenceSinceDisclosure();
    sinceDisclosure.encodedPadded <== sinceDisclosurePadded;
    sinceDisclosure.encodedLength <== sinceDisclosureLength;
    sinceDisclosure.decodedLength <== sinceDisclosureJsonLength;
    sinceDisclosure.saltLength <== sinceDisclosureSaltLength;

    // RFC 9901 requires a fresh salt for every claim. Both salts are hidden,
    // but the relation rejects reuse by requiring at least one character to
    // differ. Canonical 16-byte base64url spelling is enforced in each parser.
    component saltCharEqual[22];
    signal allSaltCharsEqual[23];
    allSaltCharsEqual[0] <== 1;
    for (var i = 0; i < 22; i++) {
        saltCharEqual[i] = IsEqual();
        saltCharEqual[i].in[0] <== municipalityDisclosure.saltChars[i];
        saltCharEqual[i].in[1] <== sinceDisclosure.saltChars[i];
        allSaltCharsEqual[i + 1] <== allSaltCharsEqual[i] * saltCharEqual[i].out;
    }
    allSaltCharsEqual[22] === 0;

    signal municipalityDigestPadded[44];
    signal sinceDigestPadded[44];
    for (var i = 0; i < 43; i++) {
        municipalityDigestPadded[i] <== municipalityDisclosureDigestB64[i];
        sinceDigestPadded[i] <== sinceDisclosureDigestB64[i];
    }
    municipalityDigestPadded[43] <== 0;
    sinceDigestPadded[43] <== 0;
    component decodedMunicipalityDigest = DecodeSD(44, 32);
    decodedMunicipalityDigest.sdBytes <== municipalityDigestPadded;
    decodedMunicipalityDigest.sdLen <== 43;
    component decodedSinceDigest = DecodeSD(44, 32);
    decodedSinceDigest.sdBytes <== sinceDigestPadded;
    decodedSinceDigest.sdLen <== 43;
    component canonicalMunicipalityDigest = AssertBase64UrlCanonical32();
    canonicalMunicipalityDigest.encoded <== municipalityDisclosureDigestB64;
    component canonicalSinceDigest = AssertBase64UrlCanonical32();
    canonicalSinceDigest.encoded <== sinceDisclosureDigestB64;
    for (var i = 0; i < 32; i++) {
        decodedMunicipalityDigest.base64Out[i] === municipalityDisclosure.digestBytes[i];
        decodedSinceDigest.base64Out[i] === sinceDisclosure.digestBytes[i];
    }
    ...
    splitShared[2] <== municipalityDisclosure.municipalityBfs
        + sinceDisclosure.residenceSinceDay1900 * 8192;
