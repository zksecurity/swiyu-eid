...
/// Online proof of an issuer-authenticated current main-residence relationship.
/// The exact BFS municipality and eCH-0011-style arrival date stay private.
template SwiyuResidenceShowSplit(statusDepth) {
    ...
    signal input statusSiblings[statusDepth][2][32];
    ...
    component municipalityPositive = GreaterEqThan(13); municipalityPositive.in[0] <== municipalityBfs; municipalityPositive.in[1] <== 1; municipalityPositive.out === 1;
    ...
    // Bind the business policy to elapsed whole UTC days.  1970-01-01 is day
    // 25,567 relative to 1900-01-01.
    ...
    component secondBound = LessThan(17); secondBound.in[0] <== secondsInDay; secondBound.in[1] <== 86400; secondBound.out === 1;
    signal currentDay1900 <== unixDay + 25567;
    component currentDayBits = Num2Bits(18); currentDayBits.in <== currentDay1900;
    component supportedDateMax = LessEqThan(18); supportedDateMax.in[0] <== currentDay1900; supportedDateMax.in[1] <== 109572; supportedDateMax.out === 1;

    component minimumBits = Num2Bits(12); minimumBits.in <== minimumResidenceDays;
    component minimumPositive = GreaterEqThan(12); minimumPositive.in[0] <== minimumResidenceDays; minimumPositive.in[1] <== 1; minimumPositive.out === 1;
    component minimumMax = LessEqThan(12); minimumMax.in[0] <== minimumResidenceDays; minimumMax.in[1] <== 3650; minimumMax.out === 1;
    ...
    component countMin = GreaterEqThan(5); countMin.in[0] <== allowedCount; countMin.in[1] <== 1; countMin.out === 1;
    component countMax = LessEqThan(5); countMax.in[0] <== allowedCount; countMax.in[1] <== 16; countMax.out === 1;
    component enabled[16];
    component equal[16];
    signal matches[17];
    matches[0] <== 0;
    for (var i = 0; i < 16; i++) {
        enabled[i] = LessThan(5); enabled[i].in[0] <== i; enabled[i].in[1] <== allowedCount;
        equal[i] = IsEqual(); equal[i].in[0] <== municipalityBfs; equal[i].in[1] <== allowedMunicipalityCodes[i];
    ...
    component statusProof = SwiyuTernaryStatusMerkle(statusDepth);
    ...
    // Eleven ternary levels have 177,147 leaves, but this profile deliberately
    // retains the existing Swiss resolver's 131,072-entry status-list cap.
    component statusProfileLengthMax = LessEqThan(32);
    statusProfileLengthMax.in[0] <== statusListLength;
    statusProfileLengthMax.in[1] <== 131072;
    statusProfileLengthMax.out === 1;
