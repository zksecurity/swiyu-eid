...
include "swiyu/canton-disclosure.circom";
include "swiyu/p256.circom";
include "swiyu/split-commitments.circom";

/// Canton-eligibility Prepare relation derived from the optimized swiyu
/// Prepare circuit. This intentionally remains an isolated experimental
/// entrypoint until the authenticated disclosure extractor is generalized.
///
/// Credential authentication stage:
...
///   * strict issuer-authenticated `resident_canton` SD-JWT disclosure
///   * signed holder key, validity bounds, and status-list coordinates
///   * stable lookup and status-URI commitments for online Show proofs
template SwiyuCantonPrepareBase(
    maxMessageLength,
    maxB64HeaderLength,
    maxB64PayloadLength
) {
    ...
    signal cantonKey[15];
    var CANTON_KEY[15] = [114,101,115,105,100,101,110,116,95,99,97,110,116,111,110];
    for (var i = 0; i < 15; i++) cantonKey[i] <== CANTON_KEY[i];
    component noClearCanton = JsonForbiddenKeyAtDepth(maxPayloadLength, 15, 1);
    noClearCanton.json <== payload;
    noClearCanton.inStringBefore <== payloadStructure.inStringBefore;
    noClearCanton.depthBefore <== payloadStructure.depthBefore;
    noClearCanton.keyBytes <== cantonKey;
    ...
    component disclosure = SwiyuCantonDisclosure();
    ...
    splitShared[2] <== disclosure.cantonCode;
