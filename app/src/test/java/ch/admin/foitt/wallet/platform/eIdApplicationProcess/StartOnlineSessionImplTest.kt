...
class StartOnlineSessionImplTest {
    ...
    private lateinit var useCase: StartOnlineSessionImpl
    ...
    fun setUp() {
        ...
        coEvery {
            mockSIdRepository.startOnlineSession(
                caseId = any(),
                clientAttestation = any(),
    ...
    @Test
    fun `A start online session failure is propagated`() = runTest {
        val exception = Exception("my exception")
        coEvery {
            mockSIdRepository.startOnlineSession(
                caseId = any(),
